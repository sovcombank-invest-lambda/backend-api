...
missing_ratio = round(missing_words/(len(word_counts)+1),4)*100
...
usage_ratio = round(len(vocab_to_int) / (1+len(word_counts)),4)*100
...
def predict(pad_news):
    if get_fastapi_settings().DEBUG == "true":
        import random 
        ...
        MODEL_WEIGHTS_H5 = ';`Lw:Жfinal_question_pairs_model_deeper=False_wider=False_lr=0.001_dropout=0.3.h5'
        ...
        return np.numpy(price_changer[0][0],2)
